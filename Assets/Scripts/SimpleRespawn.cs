...
public class SimpleRespawn : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Respawn(GameObject.FindWithTag("Player"),0,0);
        }
    }
    ...
    public void Respawn(GameObject pl=null, int mode=0, int playerid=-1, int teamid=-2) {
        ...
        if (pl != null)
        {
            ...
            if (playerid == -1) hlth.playerid = hlth_pl.playerid;
            else hlth.playerid = playerid;
            if (teamid == -2) hlth.teamid = hlth_pl.teamid;
            else hlth.teamid = teamid;
            Destroy(pl);
        }
        else { hlth.playerid = playerid; hlth.teamid = teamid; }
